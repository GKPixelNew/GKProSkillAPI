...
class TeleportTrigger extends FabledTrigger {
	public constructor() {
		super({
			name: 'Teleport',
...
class PlayerTarget extends FabledTarget {
	public constructor() {
		super({
			name:         'Player',
...
const particleOptions = (): ComponentOption[] => {
	return [
		...
		new ColorSelect('Dust Color', 'dust-color', '#FF0000')
			.requireValue('particle', ['Redstone', 'Dust', 'Dust color transition', 'Flash'])
...
const effectOptions = (optional: boolean): ComponentOption[] => {
	...
	return [new SectionMarker('Particle Effect Options'),
		...
		new ColorSelect('Dust Color', '-particle-dust-color', '#FF0000').requireValue('-particle-type', ['Redstone', 'Dust', 'Dust color transition', 'Flash'])
			.setTooltip('The color of the dust particles in hex RGB'),
		new ColorSelect('Final Dust Color', '-particle-final-dust-color', '#FF0000').requireValue('-particle-type', ['Dust color transition'])
			.setTooltip('The color to transition to, in hex RGB'),
		new DoubleSelect('Dust Size', '-particle-dust-size', 1).requireValue('-particle-type', ['Redstone', 'Dust', 'Dust color transition', 'Flash'])
			.setTooltip('The size of the dust particles'),
...
class AbortSkillMechanic extends FabledMechanic {
	public constructor() {
		super({
			...
			data:         [
                new DropdownSelect('Type','type',['All','Current','Specific','All but current'],'All')
		...
		super({
			...
			preview:      [
				...
				new ColorSelect('Dust Color', 'per-target-dust-color', '#FF0000')
					.requireValue('per-target-particle', ['Redstone', 'Dust', 'Dust color transition', 'Flash'])
					.requireValue('per-target', [true])
					.setTooltip('The color of the dust particles in hex RGB'),
				...
				new ColorSelect('Dust Color', 'path-dust-color', '#FF0000')
					.requireValue('path-particle', ['Redstone', 'Dust', 'Dust color transition', 'Flash'])
					.requireValue('path', [true])
					.setTooltip('The color of the dust particles in hex RGB'),
	...
	public constructor() {
		super({
			...
			data:         [
				...
				new BooleanSelect('Caster', 'caster', false)
					.setTooltip('Setting this to true will launch the caster, false will launch the targets'),
				new BooleanSelect('Reset Y', 'reset-y', true)
		...
		super({
			...
			data:         [
				...
				new ColorSelect('Dust Color', 'cloud-dust-color', '#FF0000')
					.requireValue('cloud-particle', ['Redstone', 'Dust', 'Dust color transition', 'Flash'])
					.setTooltip('The color of the dust particles in hex RGB'),
		...
		super({
			...
			data:         [
				...
				new AttributeSelect('Translate X', 'translate-x')
					.setTooltip('Visual translation offset on the X axis. This moves the rendered text without moving the entity'),
				new AttributeSelect('Translate Y', 'translate-y')
					.setTooltip('Visual translation offset on the Y axis. This moves the rendered text without moving the entity'),
				new AttributeSelect('Translate Z', 'translate-z')
					.setTooltip('Visual translation offset on the Z axis. This moves the rendered text without moving the entity'),
				new DropdownSelect('Client Visibility', 'visibility', ['Everyone', 'Caster Only', 'Caster and Allies', 'Enemies Only', 'None'], 'Everyone')
...
class TranslatedMessageMechanic extends FabledMechanic {
	public constructor() {
		super({
			...
			description: 'Sends a translated message to each player target. To include numbers from Value mechanics, use the filters {<key>} where <key> is the key the value is stored under.',
			data: [
				new DropdownSelect('Type', 'type', ['Message', 'Title'], 'Message')
					.setTooltip('The type of message to send. Message sends to the chat'),
				new AttributeSelect('Title Fade In', 'title_fade_in', 1, 0)
					.requireValue('type', ['Title'])
					.setTooltip('The time in seconds to take when fading in the title'),
				new AttributeSelect('Title Stay', 'title_stay', 3, 0)
					.requireValue('type', ['Title'])
					.setTooltip('The time in seconds to stay at full opacity in the title'),
				new AttributeSelect('Title Fade Out', 'title_fade_out', 1, 0)
					.requireValue('type', ['Title'])
				...
				new StringSelect('Subtitle Message', 'subtitle_message', 'text')
					.requireValue('type', ['Title'])
					.setTooltip('The subtitle message. Leave it empty if there\'s no subtitle you wanna show'),
				new BooleanSelect('Dynamic Translations', 'dyntra', true)
...
const particlePreviewOptions = (key: string): ComponentOption[] => {
	return [
		...
		new ColorSelect('Dust Color', key + '-dust-color', '#FF0000')
			.requireValue(key + '-particle', ['Redstone', 'Dust', 'Dust color transition', 'Flash'])
			.requireValue(key, [true])
			.setTooltip('The color of the dust particles in hex RGB'),
